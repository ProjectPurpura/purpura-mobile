...
public class UpdateProduct extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        productImageView = findViewById(R.id.updateProductImage);
        ImageView backButton = findViewById(R.id.updateProductBackButton);
        Button updateButton = findViewById(R.id.updateProductAddProductButton);
        ...
        updateButton.setOnClickListener(v -> {
            Residue updatedResidue = new Residue(
            ...
            try{
                service.updateResidue(cnpj, productId, updatedResidue, this).enqueue(new Callback<Residue>() {;
                    @Override
                    public void onResponse(Call<Residue> call, Response<Residue> response) {
                        if (response.isSuccessful()) {
                            Toast.makeText(UpdateProduct.this, "Produto atualizado com sucesso!", Toast.LENGTH_SHORT).show();
                            methods.openScreenActivity(UpdateProduct.this, UpdateProduct.class);
                        }
                    }

                    @Override
                    public void onFailure(Call<Residue> call, Throwable t) {
                        Toast.makeText(UpdateProduct.this, "Erro na requisição: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            }catch (Exception e){
            ...
            service.getResidueById(cnpj, getIntent().getStringExtra("residueId")).enqueue(new Callback<Residue>() {
                ...
                public void onResponse(Call<Residue> call, Response<Residue> response) {
                    ...
                    if (residue != null) {
                        ...
                        productId = residue.getId();
                        adressId = residue.getIdEndereco();
                        pixKeyId = residue.getIdChavePix();

                        Glide.with(UpdateProduct.this)
                                .load(residue.getUrlFoto())
                                .into(productImageView);
                    } else {
                        Toast.makeText(UpdateProduct.this, "Falha ao carregar o produto.", Toast.LENGTH_SHORT).show();
                    }
                }
